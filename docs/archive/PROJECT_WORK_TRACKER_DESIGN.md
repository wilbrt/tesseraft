...
- [`deterministic-code-review-loop`](../../examples/catalog/deterministic-code-review-loop/workflow.edn)

Related current contracts:

- [`PROJECTS.md`](../reference/PROJECTS.md)
- [`CONTROL_PLANE_API.md`](../reference/CONTROL_PLANE_API.md)
- [`WORKFLOW_RUNS.md`](../reference/WORKFLOW_RUNS.md)
- Removed v1 `schemas/project.schema.json` (historical contract)
...
dependency-ordered increments. Every phase uses `deterministic-code-review-loop`
...
```text
WT1 Project identity, discovery, registry, and migration          [Deterministic review]
 └── WT2 Credential stores + recursive secret safety             [Deterministic review]
      └── WT3 Optional primary work-tracker contract             [Deterministic review]
           └── WT4 Settings UI + Connections Doctor              [Deterministic review]
                └── WT5 Normalized read boundary + Plane          [Deterministic review]
                     ├── WT6 Jira + GitHub Issues read adapters    [Deterministic review]
                     │    └── WT6I Generic intake integration      [Deterministic review]
                     └── WT7 Plane bootstrap/sync                  [Deterministic review]
```
...
Every increment uses `deterministic-code-review-loop`. Start from a clean base
branch containing all dependencies.

```bash
./bin/tesseraft run start examples/catalog/deterministic-code-review-loop/workflow.edn \
  --run-id <id> \
  --input repo-root=. \
  --input base-branch=main \
  --input prompt='<paste one deterministic-code-review-loop prompt below>' \
  --format json
```
...
push/PR side effect. See [`WORKFLOW_RUNS.md`](../reference/WORKFLOW_RUNS.md).

---

## WT1 — Project identity, discovery, registry, and migration

**Delivery workflow:** `deterministic-code-review-loop`
...
### Deterministic code-review-loop prompt

```text
Implement WT1 from docs/archive/PROJECT_WORK_TRACKER_DESIGN.md with
deterministic-code-review-loop.
...
```text
Implement WT2 from docs/archive/PROJECT_WORK_TRACKER_DESIGN.md with deterministic-code-review-loop
...
```text
Implement WT3 from docs/archive/PROJECT_WORK_TRACKER_DESIGN.md with deterministic-code-review-loop
...
```text
Implement WT4 from docs/archive/PROJECT_WORK_TRACKER_DESIGN.md with
deterministic-code-review-loop after WT3. Add a schema-driven primary work-tracker
...
```text
Implement WT5 from docs/archive/PROJECT_WORK_TRACKER_DESIGN.md with deterministic-code-review-loop
...
```text
Implement WT6 from docs/archive/PROJECT_WORK_TRACKER_DESIGN.md with deterministic-code-review-loop
...
```text
Implement WT6I from docs/archive/PROJECT_WORK_TRACKER_DESIGN.md with deterministic-code-review-loop
...
```text
Implement WT7 from docs/archive/PROJECT_WORK_TRACKER_DESIGN.md with deterministic-code-review-loop
