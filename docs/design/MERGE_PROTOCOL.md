...
`examples/catalog/pr-housekeeping/workflow.edn` is a follow-up; the protocol gates
are unchanged.)

## Validation

- `./bin/tesseraft lint examples/catalog/pr-housekeeping/workflow.edn` — the workflow
...
New:
- `docs/design/MERGE_PROTOCOL.md` (this file).

Modified:
- `examples/catalog/pr-housekeeping/scripts/classify_prs.py` — adds `age_days`,
  `rebase_recommended`, the `recommend-rebase` action bucket, and an Age
  column / rebase-recommendation count in `report.md`.
- `examples/catalog/pr-housekeeping/scripts/plan_actions.py` — adds the
...
- `examples/catalog/pr-housekeeping/workflow.edn` — no new node; the runtime stays
