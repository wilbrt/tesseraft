...
| Run state | `<runs-root>/<workflow>/<run>/state.edn` | runtime and run-query service | runtime lifecycle service only | `schemas/run-state.schema.json` and version codec | `tesseraft.migration.runs` | Older versions decoded at the boundary; no synthesized mutable state |
...
| Approvals | Run-local approval request, decision, and comment records | runtime, approval service, Web UI | approval application service only | durable approval/event contracts | `tesseraft.migration.runs` | API aliases delegate to the canonical project-scoped operation |
