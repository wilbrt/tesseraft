...
`examples/catalog/focused-tdd-to-pr/workflow.edn` is a lightweight route from a prompt to a reviewed pull request. Focused TDD is an implementation discipline inside one coherent execution state, not a scenario-by-scenario workflow state machine.
...
```bash
./bin/tesseraft run start examples/catalog/focused-tdd-to-pr/workflow.edn \
  --run-id <id> \
