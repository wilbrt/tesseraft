...
Status: Accepted — bounded runtime surface documented below
...
This document distinguishes the implemented P1.4 surface from the target executable contract. The ordered implementation prompts are in [FRAGMENT_IMPLEMENTATION_PROMPTS.md](../archive/FRAGMENT_IMPLEMENTATION_PROMPTS.md).
...
| Durable resume, cancellation, and orphan recovery across interruption | Implemented for non-approval fragments: a killed process continues an in-flight nested run from its persisted `state.edn` instead of restarting it, every internal event is mirrored live into the parent log under a `fragment.*` namespace, `run cancel` cancels non-terminal nested runs alongside the parent, and a killed internal node is durably orphaned rather than silently re-run |
...
- `examples/catalog/fragments/<name>/fragment.edn`
...
A fragment node is atomic in the sense that a single fragment step is one parent-visible unit of routing (a `resume --max-steps 1` still completes the whole nested run when uninterrupted), but as of FI8 it is no longer atomic under interruption: the nested run dir written by `internal-context`/`pin!` (`state.edn`, `events.jsonl`, `pin.json`) is a full durable run in its own right, so `run-fragment-node!` continues it from wherever it was left rather than recreating it. Re-entering a `:fragment` node checks `fragments/<state>/<attempt>/state.edn` first:

Both durable branches below call `verify-pin!` first, before checking whether the nested run reached a terminal status: `finish!` on the terminal branch reads the reached exit entry and required-output contract from the *current* package on disk (`(get-in pkg [:fragment :exit])`, `(get-in pkg [:interface :outputs])`), not a snapshot, so a package edited after the nested run finished but before the parent recorded it must be refused the same way an in-flight resume is, rather than mapping the outcome or materializing exit outputs through the changed package.

1. no durable nested run yet -> the original fresh path (pin, isolated context, `run-until-done!` from the internal graph's `:initial` state);
2. a durable nested run that already reached its own terminal status -> `verify-pin!`, then skip straight to `finish!` (covers a process killed after the nested run finished but before the parent recorded it);
3. a durable nested run still in flight -> `verify-pin!` refuses to continue if the package's content hash has changed since the original pin (`fragment_pin_changed`), then a parent `fragment.resumed` event is appended and `run-until-done!` continues the internal graph from its persisted `:run :state`/`:round`/`:attempt`, through the exact same `step!` the top-level runtime uses -- including its existing agent-artifact recovery, so an internal `:agent` node interrupted after writing its status artifact but before the process returned is recovered, not re-invoked, and its existing orphan fail-fast, so an internal `:process`/`:timer`/`:deterministic` node interrupted mid-execution durably orphans the nested run (`node.orphaned`) instead of silently re-running. The top-level `step!` also stops treating a `:fragment` node's own un-terminated `node.started` as an orphan once a durable nested run exists for it, so the parent step itself resumes instead of failing first.

Every event the nested loop appends (`node.*`, `transition.selected`, `effect.applied`, `run.*`) is mirrored live into the parent's own `events.jsonl` as it happens (`tesseraft.runtime.store/event!`, not a copy taken only at `finish!`), so the parent log alone proves what happened inside the boundary even if the process dies mid-fragment: the mirrored `:event` is namespaced `fragment.<original>` (e.g. `fragment.node.started`), the parent's own `:state`/`:attempt` identify which inclusion attempt it belongs to, and the nested event's own `:state`/`:attempt` (when it has them) are preserved under `:internal_state`/`:internal_attempt`. `fragment.started`/`fragment.finished`/`fragment.resumed` remain parent-native (not mirror copies). The control plane's `get-run` reverses this transform per parent attempt to derive `:internal_attempts` with `derive-attempts-from-events` reused verbatim, and `scan-artifacts` now includes `fragments` so nested `state.edn`/`events.jsonl`/`pin.json` read as artifacts of the parent run.

`run cancel` extends the same way: after reaping the parent's own runtime process and owned processes, the parent's own `"cancelled"` status is durably saved *first*, then every non-terminal `fragments/*/*/state.edn` is marked `"cancelled"`, mirroring a `run.cancelled` into the parent log through the nested ctx's own persisted `:event-mirror` -- a nested run a killed parent process left running does not stay silently `"running"` forever. Cancelling nested runs is best-effort per attempt directory: a throw reading or writing one nested `state.edn` (e.g. unreadable or truncated) is caught and logged rather than propagated, so it can never leave the parent's own already-persisted cancellation unrecorded, and it never stops the remaining nested attempt directories from being cancelled.
...
- template-visibility of materialized exit outputs to parent nodes (FI8/FI10 — deferred; FI8 stayed inside the boundary);
- a per-node timeout engine inside a fragment (internal nodes keep top-level parity: cancellation is the enforcement path, not a bounded internal step budget);
...
```text
examples/catalog/fragments/test-fix-loop/
  fragment.edn
  prompts/fix.md.tmpl
...
Runtime behavior is proved by fixtures under `test/fixtures/valid/fragment-runtime/`, split across the FI6, FI7, and FI8 focused test files:
...
test/fixtures/valid/fragment-runtime/
  ...
  runtime-resume/fragment.edn                  ; a :process :mark then an :agent :execute (:executor :pi-cli) writing agent/status.json
  runtime-hang/fragment.edn                    ; a :process :mark then a :process :hang that writes a pidfile and sleeps
```
...
`test/fragment_runtime_nodes.test.py` (FI7) stages `runtime-timer`, `runtime-process`, `runtime-mock-agent`, `runtime-exit-outputs`, `runtime-exit-outputs-missing`, and `runtime-retry-loop` with `shutil.copytree` (preserving the script executable bit) and asserts: the timer fixture actually sleeps and routes on its declared outcome; the process fixture runs its script from the fragment package dir (not the workflow dir) and writes its required output into the nested run dir; the mock-agent fixture, started with `--executor mock`, renders the package-relative prompt template into the nested run dir and writes a placeholder status artifact with no credentials; the exit-outputs fixture materializes its required output at the inclusion's declared `:prefix`, recording ownership in `fragments/exit-index.json`; two inclusions of that same fixture at two distinct, non-colliding prefixes both materialize and stay independently readable at their own prefixed path, each recorded under its own key in `fragments/exit-index.json`; two inclusions of that same fixture with no distinct prefixes fail the second one durably with `fragment_exit_output_conflict` while leaving the first inclusion's projected artifact untouched; a blank-prefix inclusion refuses to overwrite a pre-existing, unowned parent-level artifact at its exit-output destination, also with `fragment_exit_output_conflict` (a null `owner_state`); a parent that loops back into the very same inclusion state re-materializes its exit output a second time without conflict, since a state re-entering on its own retry owns the destination it already owns; a `:produces` value that fails `spec/safe-relative-path?` is refused by `materialize-exit-outputs!` with `fragment_exit_output_invalid_path` before anything is read or copied, proven by calling the runtime function directly so the refusal holds independent of the matching `fragment-exit-invalid-produces-path` package-lint rule; the exit-outputs-missing fixture completes its nested run but fails the parent durably with `fragment_exit_output_missing` because the declared exit path was never written, and projects nothing; and the retry-loop fixture's internal round count reaches exactly its own `:max-rounds` before routing, while the parent's own round stays unchanged. `test/cli_contracts.test.py` auto-discovers and lints every valid fragment fixture, and checks unsafe exit paths in normal and strict modes.

`test/fragment_runtime_recovery.test.py` (FI8) stages `runtime-resume` and `runtime-hang`, launches `run resume` in the background, polls for durable evidence that a nested node is in flight (the agent stub's `agent/status.json`, or the hang script's own pidfile), and SIGKILLs the pid recorded in the parent's `runtime-process.json` to reproduce a genuine mid-fragment interruption. It asserts: a forced restart mid `:execute` (an `:agent` node whose `PI_BIN` stub writes its required status artifact before sleeping) resumes with a `fragment.resumed` parent event and a nested `node.recovered`, never re-invoking the stub (its call-count file stays at one line) and never re-running the already-finished `:mark` process (`marks.txt` stays at one line), reaching `"done"`; a forced restart mid `:hang` (a `:process` node with no recovery path) durably orphans the nested run — a nested `node.orphaned` for state `hang`, mirrored into the parent log as `fragment.node.orphaned` with `:internal_state "hang"`, and a parent `node.failed` classified `fragment_internal_failure` — while `marks.txt` stays unchanged, proving no completed effect is replayed; and `run cancel` issued live while `:hang` sleeps leaves both the parent's and the nested run's own `state.edn` `"cancelled"`, mirrors a `fragment.run.cancelled` into the parent log, reaps the hang process, and surfaces `:internal_attempts` on the corresponding parent attempt from `GET`-equivalent `control-plane run <id>`. Two further cases exercise the durable branches without a live kill, by running `runtime-resume` once to full, uninterrupted completion in an isolated seed workspace and copying its real completed nested attempt directory onto a *different*, freshly started parent run's `fragments/run-fragment/1/` (rewriting the seed's embedded absolute nested-run-dir path to the destination's) so that run's very first look at that state+attempt already finds a durable nested run: resuming an unmodified copy takes the durable-terminal branch straight to `finish!` — no `fragment.resumed`, the nested run's own `events.jsonl` and `marks.txt` byte-for-byte unchanged, reaching `"done"` — while resuming a copy whose staged `fragment.edn` was edited after the seed's `pin!` fails durably with a parent `node.failed` classified `fragment_pin_changed`, before any `fragment.finished` and without touching the nested run again.

Focused tests in `test/cli_contracts.test.py` cover valid lint, strict lint, malformed interfaces/exits/assets, internal graph checks, inclusion input/outcome diagnostics, and transactional authoring import followed by strict lint of the resulting workflow.
...
Each increment and its ready-to-run Canon TDD prompt is specified in [FRAGMENT_IMPLEMENTATION_PROMPTS.md](../archive/FRAGMENT_IMPLEMENTATION_PROMPTS.md).
