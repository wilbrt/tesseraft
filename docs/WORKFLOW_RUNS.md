...
`review-loop` uses the same state shape, with explicit pass/fail execution, UI-quality, and code-review artifacts. After implementation it starts a worktree-rooted server, deterministically captures desktop/compact/mobile/open-overlay screenshots and geometry, runs functional browser testing, then runs an independent screenshot-only visual review. A deterministic validator rejects passes with missing evidence, incomplete checks, actionable findings, or contradictory "minor/non-blocking/workaround" prose. Passing screenshot sets are committed under `review-evidence/<run-id>/` on the implementation branch and linked from the PR body for human review. Declared `status: fail` outcomes merge issues, clean up the server, increment the round, and return to `execute` until the workflow passes or reaches a declared failure path. That expected outcome is distinct from an external/runtime failure such as a missing dependency, subprocess crash, malformed output, timeout, or missing required artifact; external failures leave durable `node.failed` evidence and require explicit recovery/resume/retry.
