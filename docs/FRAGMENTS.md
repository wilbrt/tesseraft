...
2. a durable nested run that already reached its own terminal status -> skip straight to `finish!` (covers a process killed after the nested run finished but before the parent recorded it);
...
`run cancel` extends the same way: after reaping the parent's own runtime process and owned processes, every non-terminal `fragments/*/*/state.edn` is marked `"cancelled"`, mirroring a `run.cancelled` into the parent log through the nested ctx's own persisted `:event-mirror` -- a nested run a killed parent process left running does not stay silently `"running"` forever.
