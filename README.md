...
- **fragment-runtime-execution** (implemented) — Bounded deterministic runtime for `{:type :fragment}` nodes: per-execution re-resolution and pin of the package identity/content hash with the parent run, rendered input/effective-parameter binding into an isolated nested run context, execution of reachable `:deterministic`/`:router`/`:terminal` internal states through the existing handler/transition machinery in a single atomic parent step, fragment-local `:max-rounds` enforcement, mapping of the reached terminal outcome onto the parent `:fragment/outcome` transition, and durable inspectable nested `pin.json`/`state.edn`/`events.jsonl` under `fragments/<state>/<attempt>`. A reachable `:agent`/`:process`/`:timer`/`:approval`/nested-`:fragment` internal node is rejected with `fragment_unsupported_node` before any internal execution; an unresolvable inclusion fails with `fragment_unresolved`; exhausting the fragment's own rounds fails with `fragment_max_rounds` without advancing the parent round.
  _Evidence:_ src/tesseraft/runtime/fragment.clj, src/tesseraft/runtime/core.clj run-fragment-node!/execute-node!/match-transition?, test/fixtures/valid/fragment-runtime/{runtime-pass,runtime-fail,runtime-rounds,runtime-timer}/fragment.edn, test/fragment_runtime_execution.test.py, docs/FRAGMENTS.md#runtime-behavior, scripts/test.sh FI6
