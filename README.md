...
- **work-tracker-plane-read** (implemented) — Read-only provider-neutral `:work-tracker/fetch-item` boundary for Plane: resolves the selected run project's persisted `connections.work-tracker`, uses only the project-scoped credential ref, performs an injectable bounded API-key request, and persists a versioned normalized work-item artifact without raw Plane payloads or secrets. Mock mode remains offline.
