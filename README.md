...
- **work-tracker-contract** (implemented) — Optional project-owned `connections.work-tracker` contract with normalized provider/credential-ref/config envelope, built-in Plane/Jira/GitHub Issues schema validation, CLI/HTTP set-inspect-clear behavior, recursive secret rejection, atomic project-scoped persistence, and no provider API execution. Legacy GitHub code-host/PR and Jira connection semantics remain independent.
  _Evidence:_ src/tesseraft/control_plane/core.clj normalize-work-tracker/update-project-connections, src/tesseraft/control_plane/cli.clj --work-tracker-* and --clear-work-tracker, web/src-server/routes/api.ts work_tracker forwarding, schemas/work-tracker*.schema.json, schemas/project.schema.json connections.work-tracker, schemas/portable-project-descriptor.schema.json connections.work-tracker, test/work-tracker-contract.test.js, docs/PROJECTS.md Work tracker connection, docs/CONTROL_PLANE_API.md project connections work_tracker
