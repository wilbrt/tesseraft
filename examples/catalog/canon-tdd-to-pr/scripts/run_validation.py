#!/usr/bin/env python3
"""Run Canon TDD check manifests and return a declared workflow outcome.
...
def artifact_paths(phase: str, round_number: int) -> tuple[str, str]:
    if phase == "regression":
        return (f"manual-testing/report-{round_number}.md", f"manual-testing/issues-{round_number}.json")
