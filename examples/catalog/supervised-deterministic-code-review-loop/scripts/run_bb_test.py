...
Note: `bb test` runs the repository's Clojure, Python, Node.js, and generated-
documentation checks. We invoke it in the isolated worktree so the gate tests
the agent's actual checkout, not the developer's working repo. The worktree
has its own `examples/`, `test/`, and `bb.edn`, so relative paths resolve there.

## Self-healing the STATUS.edn ↔ README.md sync drift

`bb test` runs `bb test:docs`, which fails when README's generated STATUS
section is out of sync with STATUS.edn. The implement
...
loop, this gate self-heals the one derived artifact it owns:
  ...
  - when `bb test` fails AND the failure is specifically the sync check
    ...
    `bb status` in the worktree to regenerate all status outputs from STATUS.edn,
    commits only those outputs plus STATUS.edn on the worktree branch, and re-runs
...
def regenerate_status_in_worktree(worktree: pathlib.Path) -> bool:
    """Run `bb status` in the worktree to regenerate status outputs from STATUS.edn.

    Returns True if any generated output changed. Never raises: a regeneration
    ...
    try:
        ...
        return False
    diff = _git(worktree, "diff", "--name-only", "--", *GENERATED_STATUS_PATHS)
...
def commit_status_sync(
    ...
    *,
    ...
    """Commit generated status outputs together with STATUS.edn.

    Only STATUS.edn and declared generated outputs are staged.
    Returns True if a commit was created. Never raises.
    """
    _git(worktree, "add", "STATUS.edn", *GENERATED_STATUS_PATHS)
    ...
    commit = _git(
        ...
        "--", "STATUS.edn", *GENERATED_STATUS_PATHS,
    )
    return commit.returncode == 0


def self_heal_status_sync(worktree: pathlib.Path) -> bool:
    """Regenerate and commit status outputs. Returns True if output changed."""
    ...
    if healed:
        if passed:
            note = (
                ...
                "gate ran `bb status` to regenerate status outputs from STATUS.edn, "
                "committed STATUS.edn and generated status outputs on this worktree branch, and "
