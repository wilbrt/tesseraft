...
def main():
    ...
    target = int(inputs.get("target-pr") or (run_dir / "comment-repair" / "target-pr.txt").read_text().strip())
    ...
    head = pr["headRefName"]
    repair_branch = f"pr-housekeeping/comments-{target}-{safe_component(head)}-{safe_component(request['run']['id'])}"
    worktree = repo_root / ".agent-worktrees" / f"pr-housekeeping-comments-{safe_component(request['run']['id'])}-pr-{target}"
    ...
    metadata = {
        ...
        "target_pr": target,
        "head_ref": head,
        "base_ref": pr.get("baseRefName") or inputs.get("base-branch") or "main",
    ...
    json.dump({"ok": True, "status": "pass", "branch": repair_branch, "worktree-dir": str(worktree), "outputs": {"worktree": "comment-repair/worktree.json", "worktree-path": "comment-repair/worktree-path.txt", "repair-branch": "comment-repair/repair-branch.txt"}}, sys.stdout)
    print()
