...
function cp(args) {
  return JSON.parse(execFileSync('bb', ['-m', 'tesseraft.migration.cli', ...args], {
    cwd: repoRoot,
    encoding: 'utf8',
...
test('SC-005 migrates flat legacy local credentials without mutating source and is repeatable', () => {
  ...
  const preview = cp(['credentials', '--dry-run', '--tesseraft-home', home, '--legacy-file', legacy]);
  assert.equal(preview.state, 'pending');
  assert.equal(preview.ok, true);
  assert.equal(fs.existsSync(path.join(home, 'credentials.json')), false, 'dry-run must not write the destination');

  const first = cp(['credentials', '--apply', '--tesseraft-home', home, '--legacy-file', legacy]);
  ...
  const repeat = cp(['credentials', '--apply', '--tesseraft-home', home, '--legacy-file', legacy]);
...
test('SC-005 migrates the actual legacy credentials.json path with a non-destructive backup and remains resolvable', () => {
  ...
  const first = cp(['credentials', '--apply', '--tesseraft-home', home, '--legacy-file', dest]);
  assert.equal(first.status, 201);
  assert.equal(first.state, 'migrated');
  ...
  const repeat = cp(['credentials', '--apply', '--tesseraft-home', home, '--legacy-file', dest]);
  assert.equal(repeat.status, 200);
  assert.equal(repeat.state, 'unchanged');
...
test('SC-005 local credential migration preserves source bytes when destination writing fails', () => {
  ...
  let writeFailure;
  try {
    cp(['credentials', '--apply', '--tesseraft-home', blockedHome, '--legacy-file', legacy]);
  } catch (error) {
...
test('SC-005 local credential migration refuses invalid legacy input and destination conflicts', () => {
  ...
  let invalidFailure;
  try {
    cp(['credentials', '--apply', '--tesseraft-home', home, '--legacy-file', invalidLegacy]);
  } catch (error) {
  ...
  let conflictFailure;
  try {
    cp(['credentials', '--apply', '--tesseraft-home', home, '--legacy-file', legacy]);
  } catch (error) {
